...
const requireRedisUrl = (): string => {
  if (!env.redisUrl) throw new Error('REDIS_URL is required for asynchronous jobs');
  return env.redisUrl;
};

export const getQueueConnection = (): IORedis => {
  if (!connection) connection = new IORedis(requireRedisUrl(), { maxRetriesPerRequest: null, enableReadyCheck: false, lazyConnect: true });
  return connection;
};
