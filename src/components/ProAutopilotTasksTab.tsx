    ...
    <div className="w-full max-w-5xl mx-auto space-y-6 sm:space-y-8 animate-in fade-in duration-200">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-6">
          <div className="space-y-1">
            <h2 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight flex items-center gap-2.5">
              <span className="w-9 h-9 rounded-2xl bg-emerald-500 text-white flex items-center justify-center shadow-xs">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-slate-400 font-mono">
              共 {safeTasks.length} 个任务
            </span>
          </div>

          {safeTasks.length === 0 ? (
            <div className="py-14 text-center space-y-3 bg-slate-50/60 rounded-3xl border border-dashed border-slate-200 px-4">
            ...
            <div className="grid grid-cols-1 gap-3.5">
              {safeTasks.map((task) => {
                ...
                return (
                  <div 
                    key={task.id}
                    className={`p-5 sm:p-6 rounded-3xl border transition-all duration-150 flex flex-col md:flex-row md:items-center justify-between gap-4 ${
                      isActive 
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl animate-in zoom-in-95 duration-150">
