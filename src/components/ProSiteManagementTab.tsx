    ...
    <div className="w-full space-y-6 sm:space-y-8 animate-in fade-in duration-200">
      ...
      <div className="bg-white border border-slate-200 rounded-lg p-6 sm:p-8 shadow-2xs space-y-6">
        ...
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div className="inline-flex p-1 bg-slate-100 rounded-md text-xs font-medium self-start sm:self-auto">
              <button
                onClick={() => setActiveSiteDataFilter('ALL')}
                className={`px-3 py-1.5 rounded transition cursor-pointer ${
                  activeSiteDataFilter === 'ALL' ? 'bg-white text-slate-900 font-semibold shadow-2xs' : 'text-slate-600 hover:text-slate-900'
                }`}
              >
                全部 ({safeSites.length})
              </button>
          ...
          <div className="grid grid-cols-1 gap-4">
            {filteredSites.map((site) => {
              return (
                <div 
                  key={site.id} 
                  className="bg-white border border-slate-200/90 hover:border-slate-300 rounded-xl p-5 sm:p-6 space-y-4 shadow-2xs transition"
                >
      ...
      {editingSiteId && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white border border-slate-200 rounded-xl max-w-2xl w-full my-8 max-h-[92vh] flex flex-col shadow-xl animate-in zoom-in-95 duration-150">
            
            {/* Modal Header */}
            <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/90 flex items-center justify-between sticky top-0 z-10 rounded-t-xl">
            ...
            <div className="p-6 space-y-6 text-sm overflow-y-auto flex-1">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-700">Google Indexing API / Service Key</label>
                    <input
                      type="text"
