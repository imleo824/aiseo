...
export const ProKeywordRadarTab: React.FC<ProKeywordRadarTabProps> = ({
  ...
  const handleRunRadarScan = async (queryToScan?: string) => {
    ...
    try {
      ...
      const data = await api.serpScan({ seedKeyword: query, siteId: selectedSiteId || undefined }).catch(() => null);
      ...
      setScanError(error?.message || '关键词扫描失败，未生成任何机会词。');
    } finally {
      setIsScanning(false);
    }
  };
    ...
    <div className="w-full space-y-6 sm:space-y-8 animate-in fade-in duration-200">
      ...
      <div className="bg-white border border-slate-200/80 rounded-2xl p-4 sm:p-6 shadow-sm space-y-6">
        ...
        <div className="space-y-4">
          ...
          {scanError && (
            <div role="alert" className="rounded-xl border border-rose-200 bg-rose-50 px-3 py-2.5 text-xs text-rose-800">
              {scanError}
            </div>
          )}
