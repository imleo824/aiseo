              ...
              <div className="flex items-center justify-end gap-1.5 pt-1 border-t border-slate-100/60">
                ...
                {!isPublished && draft.status === 'PENDING_APPROVAL' && onApprovePublish && (
                  <button
                    ...
                    className="px-2.5 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-medium transition disabled:opacity-50"
                  >
                    ...
                    <div className="flex items-center justify-end gap-2">
                      ...
                      {!isPublished && draft.status === 'PENDING_APPROVAL' && onApprovePublish && (
                        <button
                          onClick={() => void handleApprovePublish(draft.id)}
                          disabled={publishingDraftId === draft.id}
                          className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-medium transition disabled:opacity-50"
                        >
                          {publishingDraftId === draft.id ? '发布中…' : '审核并发布'}
                        </button>
                      )}
            ...
            <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-end gap-2 bg-slate-50/50">
              {activeDraftModal.status === 'PENDING_APPROVAL' && onApprovePublish && (
                <button
                  ...
                  className="px-5 py-2 bg-emerald-600 text-white rounded-xl text-sm font-semibold hover:bg-emerald-700 transition disabled:opacity-50"
                >
