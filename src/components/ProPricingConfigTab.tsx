...
export const ProPricingConfigTab: React.FC<ProPricingConfigTabProps> = ({
  ...
  // Form states
  const [rate, setRate] = useState('1 USDT = 100 基础积分');
  const [trc20Address, setTrc20Address] = useState('TLv5R4q9k8YJ3Z2QxP8wK1M7n6VbC9XyZ1');
  ...
  const loadConfig = async () => {
    ...
    try {
      ...
      if (res.trc20Address) setTrc20Address(res.trc20Address);
                  ...
                  <div className="p-5 rounded-xl bg-white border border-slate-200/80 space-y-5">
                    <div>
                      ...
                      <p className="text-[11px] text-slate-400 mt-1.5">
                        用于向用户展示非固定套餐时的基准兑换公式，支持文案自主更改。
                      </p>
                    </div>

                    <div className="pt-2 border-t border-slate-100">
                      <label className="block text-xs font-bold text-slate-700 mb-1.5">
                        2. 官方收款波场网络钱包地址 (USDT - TRC20)
                      </label>
                      <input
                        type="text"
                        disabled={!isAdmin}
                      ...
                      />
                      <p className="text-[11px] text-slate-400 mt-1.5">
                        实时对接波场底层收单。修改此钱包地址后，前台用户在进行 USDT 充值时，系统显示的充值收款地址及自动生成的充值二维码将实时更替生效。
                      </p>
                    </div>
                  </div>
                </div>
