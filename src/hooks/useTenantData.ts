...
export function useTenantData(authUserId: string, activeTenantId: string, activeNav: NavItem, onTenantChange?: (newTenantId: string) => void) {
  ...
  const needsTenants = requiresWorkspaceResource(activeNav, 'tenants');
  ...
  const activeQueries = [
    accountQuery,
    sitesQuery,
    ...(needsDrafts ? [draftsQuery] : []),
    ...(needsTasks ? [tasksQuery] : []),
    ...(needsTransactions ? [transactionsQuery] : []),
    ...(needsTenants ? [tenantsQuery] : []),
